...
(function () {
  ...
  const CITY_CONFIG = {
    ...
    defaultCenter: { lat: Number("51.05463"), lng: Number("3.72194") },
    lastPosKey: "gand_lastKnownPosition",
    cityKey: "gand",
  ...
  function rebuildCategoryOptions() {
    ...
    const keys = [...counts.keys()].sort((a, b) => categoryDisplayLabel(a).localeCompare(categoryDisplayLabel(b), "fr"));
    select.innerHTML = "";
  ...
  function isWalkInProgress() {
    return (
      ...
      localStorage.getItem("gand_walkInProgress") === "true" ||
  ...
  function updateTourSavedSummary() {
    ...
    if (!el) return;
    const idxRaw = localStorage.getItem("gand_currentIndex");
    const scoreRaw = localStorage.getItem("gand_score");
