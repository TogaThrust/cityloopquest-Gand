...
function clqStorageKey(suffix) {
  const prefix = (window.CLQ_CITY && window.CLQ_CITY.storagePrefix) || 'gand';
...
let nextButton, prevButton, homeButton, cultureButton, audioBtn, poiInterestBtn, emergencyBtn, pauseBtn, stopBtn, restartBtn, quizResumeBtn;
...
function getAudioHeardIndices() {
    try {
        const saved = localStorage.getItem('gand_audio_heard_indices');
        if (saved) {
            const parsed = JSON.parse(saved);
            if (Array.isArray(parsed)) return parsed.map(Number).filter(n => !Number.isNaN(n));
        }
    } catch (_) { /* ignore */ }
    const legacy = localStorage.getItem('gand_audio_clicked_index');
...
function getAudioCompletedIndices() {
    try {
        const saved = localStorage.getItem('gand_audio_completed_indices');
        if (saved) {
            const parsed = JSON.parse(saved);
...
function markAudioHeardForIndex(idx) {
    ...
    const arr = [...set];
    try {
        localStorage.setItem('gand_audio_heard_indices', JSON.stringify(arr));
        localStorage.setItem('gand_audio_clicked_index', String(idx));
...
function clearPoiTourMuseumState() {
    ...
    localStorage.removeItem(clqStorageKey('forceTarget'));
    try {
...
function hasPendingMuseumForceTarget() {
    try {
        return Boolean(localStorage.getItem(clqStorageKey('forceTarget')));
    } catch (_) {
        return false;
    }
}
...
let completedQuizQuestions = (() => {
    const saved = localStorage.getItem("gand_completedQuizQuestions");
...
const MONS_LAST_KNOWN_POSITION_KEY = 'gand_lastKnownPosition';
...
function resetTourProgressForNewCircuit() {
  currentIndex = 0;
  localStorage.setItem('gand_currentIndex', '0');
  localStorage.removeItem('tourCompleted');
  localStorage.removeItem('gand_audio_heard_indices');
  localStorage.removeItem('gand_audio_clicked_index');
  localStorage.removeItem('gand_audio_completed_indices');
  localStorage.removeItem('gand_quizEnabled');
  localStorage.removeItem('gand_completedQuizQuestions');
  quizEnabled = false;
  quizResumeAwaitingAudio = false;
  quizPendingAdvanceCallback = null;
...
/** Variante fichier image Gand : espaces   underscores, casse conservée. */
...
function getPointImagePathCandidates(location) {
    ...
    if (location.image && !/^https?:\/\//i.test(String(location.image))) {
        if (/\.svg$/i.test(String(location.image))) {
            addLocal(String(location.image).replace(/\.svg$/i, ".jpg"));
            addLocal(String(location.image).replace(/\.svg$/i, ".jpeg"));
            addLocal(String(location.image).replace(/\.svg$/i, ".png"));
        }
...
function snapshotTourStateForExternalPage() {
    localStorage.setItem('gand_currentIndex', String(currentIndex));
    localStorage.setItem('gand_score', String(score));
    try {
...
function advanceToNextPoint() {
    ...
    if (currentIndex === filteredLocations.length - 2) {
        ...
        const visitedPoints = JSON.parse(localStorage.getItem('gand_visitedPoints') || '[]');
        if (!visitedPoints.includes(currentIndex)) {
            visitedPoints.push(currentIndex);
            localStorage.setItem('gand_visitedPoints', JSON.stringify(visitedPoints));
        }
        ...
        steps = [];
        currentStepIndex = 0;
        localStorage.setItem("gand_currentIndex", currentIndex);
        localStorage.setItem("gand_score", score);
...
function shouldResetToParcours() {
    ...
    try {
        ...
        if (localStorage.getItem('selectedCircuit')) return true;
...
function resetMainTourToParcours() {
...
async function handleResetWithCodeCheck() {
    if (isOnMainTourPage()) {
        resetMainTourToParcours();
        return;
    }

    const savedOrientationPerm = localStorage.getItem('orientationPermissionGranted');
    const forceParcoursReset = isMuseumOrSecoursGuidanceMode();
...
function clearQuizCompletionFromIndex(fromIdx) {
  ...
  localStorage.setItem('gand_completedQuizQuestions', JSON.stringify(completedQuizQuestions));
}
...
function updateQuizFooterButton() {
  ...
  const show = !guidanceMode && localStorage.getItem('gand_quizEnabled') === 'false';
...
function setQuizEnabledRuntime(enabled) {
  quizEnabled = enabled;
  localStorage.setItem('gand_quizEnabled', enabled ? 'true' : 'false');
  if (enabled) localStorage.removeItem('gand_quizStopped');
  updateQuizFooterButton();
...
function stopQuizPermanently() {
  ...
  localStorage.setItem('gand_quizStopped', 'true');
...
function showQuizPrompt(callback) {
  if (localStorage.getItem("gand_quizEnabled") !== null) {
    callback(localStorage.getItem("gand_quizEnabled") === "true");
    return;
  }
  const overlay = document.createElement('div');
  ...
  btnYes.style.cursor = 'pointer';
  btnYes.addEventListener('click', () => {
    localStorage.setItem("gand_quizEnabled", "true");
    quizEnabled = true;
    completedQuizQuestions = {};
    localStorage.removeItem('gand_completedQuizQuestions');
    localStorage.removeItem('gand_quizStopped');
    document.body.removeChild(overlay);
    updateQuizFooterButton();
    callback(true);
  });
  ...
  btnNo.style.cursor = 'pointer';
  btnNo.addEventListener('click', () => {
    localStorage.setItem("gand_quizEnabled", "false");
    quizEnabled = false;
    document.body.removeChild(overlay);
    updateQuizFooterButton();
    callback(false);
...
function initializeMainLogic() {
    ...
    const savedCurrentIndex = localStorage.getItem("gand_currentIndex");
    const savedScore = localStorage.getItem("gand_score");
    const savedQuizEnabled = freshCircuitStart
        ? null
        : localStorage.getItem("gand_quizEnabled");
    const savedCompletedQuizQuestions = freshCircuitStart
        ? null
        : localStorage.getItem("gand_completedQuizQuestions");
    ...
    const forceTarget = localStorage.getItem(clqStorageKey('forceTarget'));
    ...
    if (forceTarget) {
        ...
        const visitedPoints = safeJsonParse(localStorage.getItem('gand_visitedPoints'), []);
...
function disableFooterButtons() {
    ...
    const buttonsToDisable = [
        nextButton, prevButton, audioBtn, pauseBtn, stopBtn, restartBtn, quizResumeBtn
    ];
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    nextButton.addEventListener('click', () => {
        ...
        if (localStorage.getItem("gand_quizEnabled") === null) {
            ...
            quizEnabled = localStorage.getItem("gand_quizEnabled") === "true";
            if (quizEnabled) {
                showQuizForPointIndex(leavingIndex, advanceAfterQuiz, { force: true });
            } else {
                advanceAfterQuiz();
